    ...
    if ($settings['position'] == 'top' || $settings['position'] == 'bottom') {
        ...
        switch ($settings['nav']) {
            ...
            case 'nav':
                ...
                break;
            case 'thumbnails':
                $nav = 'uk-thumbnav uk-grid-width-medium-1-1 uk-grid-width-1-'.count($items).'';
                break;
            case 'dotnav':
...
<?php foreach ($items as $item) : ?>
    ...
    ?>
    <li<?php echo $nav_item; ?>>
        <a href=""><?php echo ($thumbnail) ? $thumbnail : $item['title']; ?></a>
        <?php if ($settings['nav'] == 'thumbnails' && $item['title'] && $settings['title'] && $settings['thumbnail_title']) : ?>
        <h4 class="uk-text-left uk-margin-top"><?php echo $item['title']; ?></h4>
        <?php endif ?>
    </li>
<?php endforeach; ?>
</ul>
